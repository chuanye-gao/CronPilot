...
This Cloudflare Worker is a narrow, authenticated egress gateway for CronPilot. It only forwards Tavily Search and Tavily Extract. It is not a general-purpose HTTP proxy.
...
Add `CRONPILOT_RELAY_KEY` and `TAVILY_API_KEY` under the Worker's runtime **Variables & Secrets**, not as build-only variables.
...
Keep `TAVILY_API_KEY` only in Cloudflare Secrets. `/health` is public and reveals no secret or provider status. All forwarding routes require the relay key.
