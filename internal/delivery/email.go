...
@media only screen and (max-width:600px){.frame{padding:16px 0 !important}.card{border-radius:0 !important;border-left:0 !important;border-right:0 !important}}`

func emailShell(content, footer string) string {
	return fmt.Sprintf(`<!doctype html><html><head><meta charset="utf-8"><style>%s</style></head><body style="margin:0;padding:0;background:#0b0e0d;color:#eef3ef;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif"><table role="presentation" width="100%%" cellpadding="0" cellspacing="0" border="0" style="width:100%%;background:#0b0e0d"><tr><td align="center" class="frame" style="padding:24px 12px"><table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:100%%;max-width:600px"><tr><td style="padding-bottom:20px;font-size:20px;font-weight:700;color:#eef3ef">CronPilot</td></tr>%s<tr><td style="padding-top:20px;text-align:center;color:#59635d;font-size:12px">%s</td></tr></table></td></tr></table></body></html>`, emailStyles, content, footer)
}
...
func executionHTML(value task.Task, run execution.Execution, result string) string {
	...
	content := fmt.Sprintf(`<tr><td class="card" style="background:#111513;border:1px solid #252b28;border-radius:14px"><table role="presentation" width="100%%" cellpadding="0" cellspacing="0" border="0"><tr><td style="padding:26px 24px;border-bottom:1px solid #252b28"><div style="color:%s;font-size:12px;font-weight:700;letter-spacing:.12em">%s</div><h1 style="font-size:24px;margin:10px 0 8px">%s</h1><p style="color:#8b9690;margin:0;line-height:1.6;font-size:14px">%s</p></td></tr><tr><td style="padding:22px 24px"><table role="presentation" width="100%%" cellpadding="0" cellspacing="0" border="0" style="font-size:13px;color:#8b9690"><tr><td style="padding:5px 0">Started</td><td style="text-align:right;color:#eef3ef">%s</td></tr><tr><td style="padding:5px 0">Finished</td><td style="text-align:right;color:#eef3ef">%s</td></tr><tr><td style="padding:5px 0">Attempts</td><td style="text-align:right;color:#eef3ef">%d</td></tr><tr><td style="padding:5px 0">Execution</td><td style="text-align:right;color:#eef3ef">%s</td></tr></table><div style="margin-top:22px;color:#68736d;font-size:11px;letter-spacing:.12em">RESULT</div><div class="result" style="margin-top:14px;font-size:15px;line-height:1.7;color:#d7deda">%s</div></td></tr></table></td></tr>`,
		statusColor, html.EscapeString(strings.ToUpper(string(run.Status))), html.EscapeString(value.Name), html.EscapeString(value.Description), html.EscapeString(run.StartedAt.Format(time.RFC1123Z)), html.EscapeString(finished), run.Attempts, html.EscapeString(run.ID), rendered)
	return emailShell(content, "Sent by CronPilot · AI work, right on time.")
}

func testEmailHTML(recipient string, sentAt time.Time) string {
	content := fmt.Sprintf(`<tr><td class="card" style="background:#111513;border:1px solid #2d3726;border-radius:14px;padding:30px 24px"><div style="width:42px;height:42px;line-height:42px;text-align:center;border-radius:50%%;background:#26320e;color:#b9f227;font-size:20px">✓</div><h1 style="font-size:26px;margin:22px 0 10px">Email delivery is ready.</h1><p style="color:#8b9690;line-height:1.7;font-size:15px">This test confirms that CronPilot can deliver task notifications to <strong style="color:#eef3ef">%s</strong>.</p><p style="color:#59635d;font-size:12px;margin-top:26px">Sent at %s</p></td></tr>`, html.EscapeString(recipient), html.EscapeString(sentAt.Format(time.RFC1123Z)))
	return emailShell(content, "Sent by CronPilot")
}

func verificationEmailHTML(name, verificationURL string) string {
	content := fmt.Sprintf(`<tr><td class="card" style="background:#111513;border:1px solid #2d3726;border-radius:14px;padding:30px 24px"><div style="color:#b9f227;font-size:12px;font-weight:700;letter-spacing:.12em">VERIFY EMAIL</div><h1 style="font-size:26px;margin:16px 0 10px">Welcome, %s.</h1><p style="color:#8b9690;line-height:1.7;font-size:15px">Confirm this email address to activate your CronPilot workspace.</p><a href="%s" style="display:inline-block;margin-top:18px;padding:13px 20px;color:#111610;background:#c8f135;border-radius:8px;text-decoration:none;font-weight:700">Verify email address</a><p style="color:#59635d;font-size:12px;margin-top:26px">This link expires in 30 minutes. If you did not create this account, ignore this email.</p></td></tr>`,
