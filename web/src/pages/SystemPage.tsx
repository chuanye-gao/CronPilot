  ...
  return (
    <div className="system-page">
      <section className="system-health panel">
        <header className="panel-header"><div><h2>{pick("运行状态与真实连接测试", "Runtime status and live checks")}</h2><p>{health?.relay_configured ? pick("Tavily 正通过 Cloudflare Relay 连接", "Tavily is routed through Cloudflare Relay") : pick("当前使用服务直连", "Providers are connected directly")}</p></div><span className={`overall-health ${health?.status === "ok" ? "healthy" : ""}`}><i />{health?.status === "ok" ? pick("系统运行正常", "All systems operational") : pick("系统需要检查", "System needs attention")}</span></header>
