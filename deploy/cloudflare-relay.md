...
CronPilot 的主应用仍部署在微信云。Cloudflare Worker 只中继 Tavily Search 和 Tavily Extract 两个固定接口，不代理任意网址，也不负责登录、任务调度、数据库或邮件。

## 1. 创建独立的中继密钥

生成一段至少 32 字节的随机字符串，作为 `CRONPILOT_RELAY_KEY`。它不是 Tavily Key，只用于微信云主服务向 Worker 证明身份。
...
然后在 Worker 的 **Settings / Variables & Secrets** 中添加两个运行时 Secret：
...
两个值都通过 Cloudflare Secret 保存，不要写入 `wrangler.jsonc`、Git 或前端代码。发布完成后先访问：
...
启用中继后，可以从微信云主服务移除 `TAVILY_API_KEY`。DeepSeek、MySQL 和 SMTP 配置不变。CronPilot 会自动把 Tavily 请求改发到 Worker。
